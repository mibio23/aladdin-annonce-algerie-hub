...
import { isArabicText, normalizeArabicDiacritics } from '../utils/distanceUtils';
...
export { isArabicText, normalizeArabicDiacritics };
...
class ArabicSearchEnhancer {
  ...
  private extractRoots(word: string): string[] {
    ...
    const normalized = normalizeArabicDiacritics(word);
    const roots: string[] = [];
  ...
  async enhanceArabicSearch(
    ...
    const normalizedQuery = opts.enableDiacriticNormalization 
      ? normalizeArabicDiacritics(query) 
      : query;
