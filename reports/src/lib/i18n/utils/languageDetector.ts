...
export function addLanguageToPath(pathname: string, language: Language): string {
  ...
  const queryIndex = pathnameString.indexOf('?');
  const hashIndex = pathnameString.indexOf('#');
  const suffixIndexCandidates = [queryIndex, hashIndex].filter((i) => i >= 0);
  const suffixIndex = suffixIndexCandidates.length > 0 ? Math.min(...suffixIndexCandidates) : -1;

  const pathOnly = suffixIndex >= 0 ? pathnameString.slice(0, suffixIndex) : pathnameString;
  const suffix = suffixIndex >= 0 ? pathnameString.slice(suffixIndex) : '';
  const normalizedPathOnly = pathOnly.startsWith('/') ? pathOnly : `/${pathOnly}`;

  const currentLanguage = extractLanguageFromPath(normalizedPathOnly);
  if (currentLanguage) {
    const replaced = normalizedPathOnly.replace(new RegExp(`^/${currentLanguage}(?=/|$)`), `/${language}`);
    return replaced + suffix;
  }

  return `/${language}${normalizedPathOnly}` + suffix;
}
