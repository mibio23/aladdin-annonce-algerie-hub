...
const PaymentSettingsPanel: React.FC = () => {
  ...
  const fetchSettings = useCallback(async () => {
    try {
      ...
      if (premiumData) {
        setPremiumSettings((premiumData.setting_value as unknown as PremiumSettings) || { premium_duration_days: 30 });
        setIsPremiumActive(premiumData.is_active);
      }
    } catch (error) {
  ...
  const saveSettings = async () => {
    try {
      ...
      const { error: premiumSaveError } = await supabase
        .from('site_settings')
        .upsert({
      ...
      if (premiumSaveError) throw premiumSaveError;

      toast({
        title: "Paramètres Premium",
        description: "La durée des annonces premium a été mise à jour",
      });
    } catch (error) {
  ...
  const updateStripeSecret = async () => {
    ...
    try {
      ...
      logger.error('Error updating Stripe secret:', error);
      toast({
        title: "Erreur",
  ...
  const updateAlgerianApiKey = async () => {
    ...
    try {
      ...
      logger.error('Error updating Algerian API key:', error);
      toast({
        title: "Erreur",
  ...
  const updateCardGatewayCredentials = async () => {
    ...
    try {
      ...
      logger.error('Error updating card gateway credentials:', error);
      toast({
        title: "Erreur",
  ...
  const updatePaypalCredentials = async () => {
    ...
    try {
      ...
      logger.error('Error updating PayPal credentials:', error);
      toast({
        title: "Erreur",
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Paramètres Premium</CardTitle>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <Label htmlFor="premium-duration">Durée Premium (jours)</Label>
              <Input
                id="premium-duration"
                type="number"
                min={1}
                value={premiumSettings.premium_duration_days}
                onChange={(e) => setPremiumSettings({ premium_duration_days: Number(e.target.value) })}
              />
            </div>
            <div className="flex items-center space-x-2">
              <Switch id="premium-active" checked={isPremiumActive} onCheckedChange={setIsPremiumActive} />
              <Label htmlFor="premium-active">Activer Premium</Label>
            </div>
          </div>
          <div className="flex items-center space-x-2">
            <Badge variant="secondary">Actuel: {premiumSettings.premium_duration_days} jours</Badge>
            <Badge variant={isPremiumActive ? "default" : "secondary"}>{isPremiumActive ? "ACTIF" : "INACTIF"}</Badge>
          </div>
          <div className="flex justify-end space-x-2 pt-4 border-t">
            <Button variant="outline" onClick={fetchSettings}>
              <RefreshCw className="w-4 h-4 mr-2" />
