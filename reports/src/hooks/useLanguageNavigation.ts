...
export const useLanguageNavigation = () => {
   ...
   */
  const navigateWithLanguage = useCallback((path: string, options?: { replace?: boolean; state?: unknown }) => {
   ...
   */
  const isCurrentLanguage = useCallback((lang: Language): boolean => {
   ...
   */
  const changeLanguageAndNavigate = useCallback((newLanguage: Language) => {
