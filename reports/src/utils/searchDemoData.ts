...
export const insertDemoSearchData = async () => {
  try {
    logger.info('Insertion des données de démonstration...');
    ...
    if (keywordsError) {
      logger.warn('Erreur lors de l\'insertion des mots-clés:', keywordsError);
    }
    ...
    if (suggestionsError) {
      logger.warn('Erreur lors de l\'insertion des suggestions:', suggestionsError);
    }
    ...
    if (queriesError) {
      logger.warn('Erreur lors de l\'insertion des requêtes:', queriesError);
    }

    logger.info('✅ Données de démonstration insérées avec succès !');
    ...
    logger.error('❌ Erreur lors de l\'insertion des données de démonstration:', error);
    return {
      success: false,
...
export const cleanDemoData = async () => {
  try {
    logger.info('Nettoyage des données de démonstration...');
    ...
    logger.info('✅ Données de démonstration nettoyées !');
