-- Script pour ajouter les attributs détaillés dans la base de données pour "Sport, Éducation & Loisirs"
-- Ce script ajoute une colonne 'attributes' (JSONB) à la table 'categories' si elle n'existe pas
-- et la remplit avec les configurations d'attributs définies pour le frontend.

-- 1. Ajouter la colonne attributes si elle n'existe pas
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'categories' AND column_name = 'attributes') THEN
        ALTER TABLE categories ADD COLUMN attributes JSONB DEFAULT '{}'::jsonb;
    END IF;
END $$;
...
}'::jsonb WHERE slug = 'animations-anniversaire';

UPDATE categories SET attributes = '{
  "destination_type": ["mer", "montagne", "campagne", "etranger"],
  "duration": ["1_semaine", "2_semaines", "1_mois"],
  "season": ["ete", "hiver", "printemps"]
}'::jsonb WHERE slug = 'colonies-vacances';

-- =============================================
-- 6. Clubs & Associations (Ajouté)
-- =============================================

-- Attributs pour la catégorie parent (L2) "Clubs & Associations"
UPDATE categories 
SET attributes = '{
  "audience": ["enfants", "adultes", "tout_public", "jeunesse", "seniors"],
  "activity_type": ["sport", "culture", "social", "educatif", "loisirs"]
}'::jsonb 
WHERE slug = 'clubs-associations';
...
WHERE slug = 'associations-culturelles';
...
WHERE slug = 'ateliers-communautaires';
...
WHERE slug = 'evenements-educatifs';
