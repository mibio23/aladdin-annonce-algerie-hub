...
const generateResponse = async (message: string, searchIntent: { searchQuery: string; filters: SearchFilters; confidence: number }, conversationHistory: ConversationMessage[]): Promise<string> => {
...
const generateFallbackResponse = (message: string, searchIntent: { searchQuery: string; filters: SearchFilters; confidence: number }): string => {
  const responses = [
...
serve(async (req) => {
  ...
  try {
    const { message, sessionId, categoryContext, language: _language, conversationHistory } = await req.json();
