...
serve(async (req) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "payment",
      success_url: `${req.headers.get("origin")}/payment-success?session_id={CHECKOUT_SESSION_ID}&announcement_id=${announcement_id}&payment_type=${payment_type}&mode=stripe`,
      cancel_url: `${req.headers.get("origin")}/payment-cancel?payment_type=${payment_type}&mode=stripe`,
      metadata: {
        payment_type,
        announcement_id,
        user_id: user.id,
