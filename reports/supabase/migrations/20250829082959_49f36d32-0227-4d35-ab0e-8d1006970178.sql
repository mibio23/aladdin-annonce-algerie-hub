...
BEGIN
  ...
  sql := format(
    'SELECT p.id, p.user_id, %s AS display_name, %s AS avatar_url, %s AS bio, %s AS location, %s AS profession, p.created_at FROM public.profiles p WHERE p.user_id = $1',
...
DROP POLICY IF EXISTS "Shop owners can manage their contacts" ON public.business_contacts;
...
DROP POLICY IF EXISTS "System only audit log access" ON public.security_audit_log;
