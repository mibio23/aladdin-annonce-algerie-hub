...
BEGIN
  ...
  IF NOT user_is_authenticated THEN
    announcement_row.phone_number := CASE 
      WHEN announcement_row.phone_number IS NOT NULL THEN '****-****' 
      ELSE NULL 
    END;
...
DROP POLICY IF EXISTS "Public can view active announcements with masked contacts" ON public.announcements;
