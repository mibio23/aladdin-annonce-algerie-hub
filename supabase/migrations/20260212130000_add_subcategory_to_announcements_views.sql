...
CREATE INDEX IF NOT EXISTS idx_announcements_subcategory_id
  ON public.announcements(subcategory_id);
...
DECLARE
  ...
  has_global_listing_number boolean;
  select_list text;
  where_clause text;
BEGIN
  SELECT EXISTS (
  ...
  SELECT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'announcements' AND column_name = 'subcategory_id'
  ) INTO has_subcategory_id;
  SELECT EXISTS (
    SELECT 1 FROM information_schema.columns
  ...
  SELECT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'announcements' AND column_name = 'global_listing_number'
  ) INTO has_global_listing_number;

  select_list := format(
    '%s AS id, %s AS title, %s AS description, %s AS price, %s AS location, %s AS image_url, %s AS image_urls, %s AS category_id, %s AS subcategory_id, %s AS created_at, %s AS updated_at, %s AS expires_at, %s AS keywords, %s AS type, %s AS condition, %s AS status, %s AS is_urgent, %s AS urgent_message, %s AS view_count, %s AS shop_name, %s AS shop_id, %s AS shop_logo_url, NULL::text AS phone_number, %s AS has_contact_info, %s AS global_announcement_number, %s AS global_listing_number',
    ...
    CASE WHEN has_subcategory_id THEN 'a.subcategory_id' ELSE 'NULL::text' END,
    ...
    CASE WHEN has_global_listing_number THEN 'a.global_listing_number' ELSE 'NULL::bigint' END
  );
...
BEGIN
  ...
  select_list := format(
    '%s AS id, %s AS title, %s AS description, %s AS price, %s AS location, %s AS image_url, %s AS image_urls, %s AS category_id, %s AS subcategory_id, %s AS user_id, %s AS created_at, %s AS updated_at, %s AS expires_at, %s AS keywords, %s AS type, %s AS condition, %s AS status, %s AS is_urgent, %s AS urgent_message, %s AS view_count, %s AS shop_name, %s AS shop_id, %s AS shop_logo_url, %s AS contact_count, NULL::text AS phone_number, %s AS phone_number_masked, %s AS global_announcement_number, %s AS global_listing_number',
    CASE WHEN has_id THEN 'a.id' ELSE 'NULL::uuid' END,
    CASE WHEN has_title THEN 'a.title' ELSE 'NULL::text' END,
    CASE WHEN has_description THEN 'a.description' ELSE 'NULL::text' END,
    CASE WHEN has_price THEN 'a.price' ELSE 'NULL::numeric' END,
