...
CREATE INDEX IF NOT EXISTS idx_favorites_announcement_id ON public.favorites(announcement_id);
...
DROP POLICY IF EXISTS "Everyone can view non-anonymous reviews" ON public.reviews;
...
USING (
  NOT is_anonymous
  OR auth.uid() = COALESCE(reviewer_id, user_id)
...
ON public.reviews 
FOR INSERT 
WITH CHECK (auth.uid() = COALESCE(reviewer_id, user_id));

CREATE POLICY "Users can update their own reviews" 
ON public.reviews 
FOR UPDATE 
USING (auth.uid() = COALESCE(reviewer_id, user_id));
