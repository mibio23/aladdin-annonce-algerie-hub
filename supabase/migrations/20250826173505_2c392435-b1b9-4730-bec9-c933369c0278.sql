...
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema = 'public'
      AND table_name = 'search_patterns_advanced'
  ) THEN
    DROP POLICY IF EXISTS "Lecture publique des patterns de recherche" ON public.search_patterns_advanced;
    DROP POLICY IF EXISTS "System only access to search patterns" ON public.search_patterns_advanced;
...
BEGIN
  IF EXISTS (
    ...
    FROM information_schema.tables
    WHERE table_schema = 'public'
      AND table_name = 'semantic_mappings'
  ) THEN
    DROP POLICY IF EXISTS "Lecture publique des mappings sémantiques" ON public.semantic_mappings;
    DROP POLICY IF EXISTS "Authenticated users can read semantic mappings" ON public.semantic_mappings;
...
BEGIN
  IF EXISTS (
    ...
    FROM information_schema.tables
    WHERE table_schema = 'public'
      AND table_name = 'auto_corrections'
  ) THEN
    DROP POLICY IF EXISTS "Lecture publique des corrections automatiques" ON public.auto_corrections;
    DROP POLICY IF EXISTS "Authenticated users can read auto corrections" ON public.auto_corrections;
...
BEGIN
  IF EXISTS (
    ...
    FROM information_schema.tables
    WHERE table_schema = 'public'
      AND table_name = 'contextual_intelligence'
  ) THEN
    DROP POLICY IF EXISTS "Intelligence contextuelle - système seulement" ON public.contextual_intelligence;
    DROP POLICY IF EXISTS "System only access to contextual intelligence" ON public.contextual_intelligence;
...
BEGIN
  IF EXISTS (
    ...
    FROM information_schema.tables
    WHERE table_schema = 'public'
      AND table_name = 'behavioral_learning'
  ) THEN
    DROP POLICY IF EXISTS "Apprentissage comportemental - insertion libre" ON public.behavioral_learning;
    DROP POLICY IF EXISTS "Session-based behavioral learning insert" ON public.behavioral_learning;
...
DROP POLICY IF EXISTS "Validated search queries insert" ON public.search_queries;
