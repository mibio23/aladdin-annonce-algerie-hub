...
async function getOrCreateConversationMemory(supabase: SupabaseClient, sessionId: string): Promise<ConversationMemory> {
  ...
  if (existing && existing.length > 0) {
    return {
      sessionId,
      messages: existing.map((conv: Record<string, unknown>) => ({
...
async function searchRelevantInformation(supabase: SupabaseClient, message: string, _categoryContext?: string) {
  const keywords = extractKeywords(message);
  const results: { announcements: Array<{ id: string; title: string; description: string; price: number | null; location: string | null; category_id: string }>; categories: Array<{ id: string; name: string; slug: string; description: string | null }>; trending: Array<{ keyword: string; trending_score: number }> } = {
    announcements: [],
    categories: [],
    trending: []
  };
...
async function analyzeUserPatterns(supabase: SupabaseClient, sessionId: string, _searchHistory: string[]) {
...
function extractCommonKeywords(searches: Array<Record<string, unknown>>): string[] {
  const keywordCounts: { [key: string]: number } = {};
  
  searches.forEach(search => {
    const keywords = extractKeywords(String((search as Record<string, unknown>).query_text || ''));
...
function extractPreferredCategories(searches: Array<Record<string, unknown>>): string[] {
  const categoryCounts: { [key: string]: number } = {};
  
  searches.forEach(search => {
    const ctx = (search as Record<string, unknown>).search_context as string | undefined;
    if (ctx && ctx !== 'general') {
      categoryCounts[ctx] = (categoryCounts[ctx] || 0) + 1;
    }
  });
...
function extractSearchTimes(searches: Array<Record<string, unknown>>): string[] {
  return searches.map(search => {
    const date = new Date(String((search as Record<string, unknown>).created_at));
    return date.getHours().toString();
  }).slice(0, 5);
}

// Générer une réponse intelligente avec GPT-4
async function generateIntelligentResponse(context: { message: string; language: string; categoryContext?: string; conversationMemory: ConversationMemory; userIntent: { type: string }; relevantData: { announcements: Array<{ id: string; title: string; price: number | null }>; categories: Array<{ id: string; name: string; slug: string; description: string | null }>; trending: Array<{ keyword: string; trending_score: number }>; }; userPatterns: { commonKeywords: string[]; preferredCategories: string[]; searchTimes: string[]; behavior: string }; userProfile?: Record<string, unknown> }) {
...
function createSystemPrompt(context: { language: string; categoryContext?: string; userPatterns?: { behavior?: string; commonKeywords?: string[] } }): string {
  const { language, categoryContext, userPatterns } = context;
  
  const prompt = `Tu es un assistant intelligent pour une marketplace au Maroc. Tu dois aider les utilisateurs dans leurs recherches et anticiper leurs besoins.
...
function createUserPrompt(context: { message: string; userIntent: { type: string; confidence?: number }; relevantData: { announcements: Array<unknown>; categories: Array<unknown>; trending: Array<{ keyword: string }> } }): string {
  const { message, userIntent, relevantData } = context;
  
  const prompt = `MESSAGE UTILISATEUR: "${message}"
...
- ${relevantData.categories.length} catégories pertinentes  
  - Tendances: ${relevantData.trending.map((t) => t.keyword).join(', ')}
...
async function saveConversationTurn(supabase: SupabaseClient, sessionId: string, userMessage: string, aiResponse: string, userIntent: { type: string }) {
...
async function updateContinuousLearning(supabase: SupabaseClient, userMessage: string, aiResponse: { content: string; confidence?: number }, userIntent: { type: string }) {
...
async function generateProactiveSuggestions(supabase: SupabaseClient, userPatterns: { commonKeywords: string[]; preferredCategories: string[] }, _categoryContext?: string) {
