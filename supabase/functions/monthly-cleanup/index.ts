...
serve(async (req) => {
  ...
  try {
    ...
    // 1b. Downgrade premium announcements whose premium_end_at has passed
    const { data: downgraded } = await supabaseClient
      .from('announcements')
      .update({ type: 'normal' })
      .lt('premium_end_at', new Date().toISOString())
      .eq('type', 'premium')
      .select('id');

    cleanupResults.premiumDowngraded = downgraded?.length || 0;
    logStep("Premium announcements downgraded to normal", { count: cleanupResults.premiumDowngraded });
