...
serve(async (req) => {
  ...
  try {
    ...
    // SECURITY: Require authentication to prevent resource abuse
...
async function resizeImage(blob: Blob, _params: { width?: number; height?: number; quality?: number }): Promise<Blob> {
...
async function createThumbnail(blob: Blob, _params: { width?: number; height?: number; quality?: number }): Promise<Blob> {
...
async function addWatermark(blob: Blob, _watermarkText: string): Promise<Blob> {
...
async function generateImageMetadata(imageBlob: Blob, processedFiles: Array<{ operation: string; size: number; filename: string }>): Promise<{ originalSize: number; originalType: string; processedAt: string; totalVariants: number; variants: Array<{ type: string; size: number; filename: string }>; performance: { totalSizeReduction: number; averageCompression: number } }> {
  return {
...
async function logImageProcessing(supabase: SupabaseClient, logData: { originalUrl: string; originalSize: number; operations: string[]; processedFiles: Array<{ size: number }>; bucket: string; processingTime: number; metadata: unknown }) {
  try {
    ...
    const logEntry = {
      ...
      total_processed_size: logData.processedFiles.reduce((acc: number, file: { size: number }) => acc + file.size, 0),
...
function calculateCompressionSavings(originalSize: number, processedFiles: Array<{ size: number }>): { originalSize: number; averageProcessedSize: number; totalSavings: number; compressionPercentage: number; estimatedBandwidthSavings: string } {
