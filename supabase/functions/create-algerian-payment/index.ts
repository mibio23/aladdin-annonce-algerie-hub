...
serve(async (req) => {
  ...
  try {
    ...
    const mockPaymentUrl = `${baseUrl}/payment-success?session_id=${paymentSession.id}&announcement_id=${announcement_id}&payment_type=${payment_type}&mode=algerian&card_type=${card_type}`;
