...
class CacheService {
  private readonly CACHE_PREFIX = 'aladdin_cache_v3_';
  ...
  set<T>(key: string, data: T, ttl: number = this.DEFAULT_TTL): void {
    ...
    try {
      ...
      logger.warn('Erreur lors du stockage en cache:', error);
  ...
  get<T>(key: string): T | null {
    ...
    try {
      ...
      logger.warn('Erreur lors de la lecture du cache:', error);
      localStorage.removeItem(this.getKey(key));
      return null;
    }
  }
  ...
  delete(key: string): void {
    ...
    try {
      ...
      logger.warn('Erreur lors de la suppression du cache:', error);
    }
  }
  ...
  clear(): void {
    ...
    try {
      ...
      logger.warn('Erreur lors du vidage du cache:', error);
    }
  }
