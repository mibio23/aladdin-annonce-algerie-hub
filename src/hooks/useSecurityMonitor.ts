...
export const useSecurityMonitor = () => {
  ...
  const validateSessionSecurity = useCallback(async (): Promise<boolean> => {
    try {
      ...
      logger.error('[Security] Session validation failed - details hidden for security');
  ...
  const validateProfileUpdate = useCallback((profileData: Record<string, unknown>): { isValid: boolean; errors: string[] } => {
    const errors: string[] = [];
    ...
    try {
      ...
      if (profileData.phone && typeof profileData.phone === 'string' && !/^\+?[0-9\-\s()]{10,20}$/.test(profileData.phone)) {
      ...
      for (const field of stringFields) {
        ...
        if (fieldValue && typeof fieldValue === 'string' && fieldValue.length > 1000) {
          errors.push(`${field} trop long - risque de sécurité`);
          
          logSecurityEvent({
            actionType: 'data_exfiltration_attempt',
            resourceType: 'profile',
            metadata: { field, length: fieldValue.length }
          });
        }
      }

    } catch {
      logger.error('[Security] Profile validation failed - details hidden for security');
