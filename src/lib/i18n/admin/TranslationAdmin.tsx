...
const collectKeys = (obj: unknown, prefix: string, keys: Set<string>) => {
  ...
  Object.keys(obj as Record<string, unknown>).forEach(key => {
    ...
    if (typeof (obj as Record<string, unknown>)[key] === 'string') {
      ...
      collectKeys((obj as Record<string, unknown>)[key], fullKey, keys);
    }
  });
};

const getNestedValue = (obj: unknown, key: string): string | null => {
...
const TranslationAdmin: React.FC<AdminProps> = ({ onClose }) => {
  ...
  const languages: Language[] = useMemo(() => ['fr', 'ar', 'en', 'de', 'es', 'it'], []);
  const domains = [
