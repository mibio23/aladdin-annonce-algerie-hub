...
export const secureLog = {
  info: (message: string, data?: unknown) => {
    if (config.environment === 'production') {
      ...
      logger.info(`[INFO] ${message}`, data ? sanitizeData(data) : '');
    }
  },
  ...
  warn: (message: string, data?: unknown) => {
    if (config.environment === 'production') {
      ...
      logger.warn(`[WARN] ${message}`, data ? sanitizeData(data) : '');
    }
  },
  ...
  error: (message: string, error?: unknown) => {
    ...
    logger.error(`[ERROR] ${message}`, sanitizedError);
  },

  debug: (message: string, data?: unknown) => {
    if (config.environment === 'development') {
      logger.debug(`[DEBUG] ${message}`, data ? sanitizeData(data) : '');
    }
  },

  security: (message: string, data?: unknown) => {
    // Security events should always be logged but with minimal data
    logger.warn(`[SECURITY] ${message}`, data ? { timestamp: new Date().toISOString() } : '');
  }
};
