...
const SidebarSearch: React.FC<SidebarSearchProps> = ({ className, categoryData: propCategoryData }) => {
  ...
  const handleItemClick = async (item: any) => {
    logger.info('Navigation vers:', item);
    ...
    if (searchQuery.trim()) {
      ...
      logger.debug('Résultats intelligents:', results);
    }
  };
      ...
      <div className={cn(
        ...
        <div className="p-4 border-b border-border flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            {isExpanded && (
              <h3 className="font-semibold text-sm">{propCategoryData ? (propCategoryData.slug === 'education-loisirs' ? t('categories.education-loisirs') : (t(`categories.${propCategoryData.slug}`) !== `categories.${propCategoryData.slug}` ? t(`categories.${propCategoryData.slug}`) : propCategoryData.name)) : t('search.category')}</h3>
            )}
          </div>
          {isExpanded && (
            <Button
            ...
            <div className="flex-1 overflow-y-auto">
              {searchQuery && searchResults.length > 0 ? (
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          <p className="font-medium text-sm">
                            {result.item.slug === 'education-loisirs' ? t('categories.education-loisirs') : result.item.name}
                          </p>
                          {result.parent && (
                            <p className="text-xs text-muted-foreground">
                              {t('search.in')} {result.parent.slug === 'education-loisirs' ? t('categories.education-loisirs') : result.parent.name}
                            </p>
                          )}
                ...
                <div className="w-full p-4 !text-center text-muted-foreground">
                  <p className="text-sm !text-center">{t('search.noResults')}</p>
                </div>
              ) : (
                <div className="p-4 space-y-4">
                  ...
                  {categoryData.map((category) => (
                    <div key={category.id} className="space-y-2">
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <span className="font-medium text-sm">{category.slug === 'education-loisirs' ? t('categories.education-loisirs') : (t(`categories.${category.slug}`) !== `categories.${category.slug}` ? t(`categories.${category.slug}`) : category.name)}</span>
                          <div className="flex items-center gap-2">
