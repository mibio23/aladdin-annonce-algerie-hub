      ...
      <div className="flex flex-wrap gap-2">
        {selectedCategory && selectedCategoryData && (
          <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900/70 text-blue-800 dark:text-blue-200">
            ...
            {selectedCategoryData.slug === 'education-loisirs' ? t('categories.education-loisirs') : (t(`categories.${selectedCategoryData.slug}`) !== `categories.${selectedCategoryData.slug}` ? t(`categories.${selectedCategoryData.slug}`) : selectedCategoryData.name)}
          </span>
        )}
