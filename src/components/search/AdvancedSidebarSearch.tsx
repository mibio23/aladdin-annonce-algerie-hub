...
const AdvancedSidebarSearch: React.FC<AdvancedSidebarSearchProps> = ({ 
      ...
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          try {
            ...
            logger.warn('Géolocalisation non disponible');
          }
        },
        () => {
          logger.warn('Géolocalisation refusée');
        }
      );
    }
  }, []);
  ...
  const generateAISuggestions = React.useCallback(async (query: string) => {
    ...
    try {
      ...
      logger.error('Erreur génération suggestions IA:', error);
    }
  }, [filters.aiAssisted, userLocation]);

  // Recherche visuelle par image
  const handleImageSearch = async (_file: File) => {
    ...
    try {
      ...
      */
      ...
      // Fallback temporaire
      toast({
        title: "Recherche visuelle désactivée",
  ...
  const runAdvancedSearch = React.useCallback(() => {
    if (!searchQuery.trim()) {
      setSearchResults([]);
      return;
    }
  ...
  }, [runAdvancedSearch, searchQuery, generateAISuggestions]);
          ...
          <div className="flex flex-col h-full pb-4">
            ...
            <div className="flex-1 overflow-y-auto">
              {searchQuery && searchResults.length > 0 ? (
                ...
                <div className="w-full p-4 !text-center text-muted-foreground space-y-2">
                  <MessageCircle className="w-8 h-8 mx-auto opacity-50" />
                  <p className="text-sm !text-center">{t('search.noResults')}</p>
                  <p className="text-xs !text-center">{t('search.adjustFilters')}</p>
                </div>
              ) : (
