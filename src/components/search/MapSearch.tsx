...
type LocationSelection = { lat: number; lng: number; address: string; wilayaCode?: string };
...
const MapSearch = ({ onLocationSelect }: MapSearchProps) => {
  ...
  const handleLocationClick = (wilayaCode: number, wilayaName: string) => {
    ...
    if (coordinates) {
      onLocationSelect({ 
        ...
        lng: coordinates.lng, 
        address: wilayaName,
        wilayaCode: wilayaCode.toString(),
      });
  ...
  const handleGeolocationSelect = (location: GeolocationCoords) => {
    const address = location.address || `${location.lat.toFixed(4)}, ${location.lng.toFixed(4)}`;
    const maybeWilayaCode =
      (typeof location.wilaya === 'string' && location.wilaya.trim()
        ? getWilayaCodeByName(location.wilaya.trim())
        : null) ?? null;
    ...
    onLocationSelect({
      ...
      lng: location.lng,
      address,
      wilayaCode: maybeWilayaCode != null ? maybeWilayaCode.toString() : undefined,
    });
