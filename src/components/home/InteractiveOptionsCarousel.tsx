...
import { useAdvertising } from '@/providers/useAdvertising';
...
const InteractiveOptionsCarousel = React.memo(() => {
  ...
  const remoteOptions = useMemo<Option[]>(() => {
  ...
  const handlePanelClick = useCallback(() => {
    const currentOption = displayOptions[activeOption];
    if (currentOption?.slug) {
      navigateWithLanguage(`/category/${currentOption.slug}`);
    }
  }, [displayOptions, activeOption, navigateWithLanguage]);
    ...
    <section className="relative w-full h-[40vh] md:h-[56vh] py-4 bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800" aria-label="Carrousel publicitaire">
      ...
      >
        <div className="relative h-full w-full flex gap-3 md:gap-4">
          {/* Panneau descriptif */}
          <article 
            className="flex-1 rounded-3xl overflow-hidden relative shadow-lg bg-black/60 text-white cursor-pointer hover:bg-black/70 transition-colors"
          ...
          {/* Panneau média */}
          <aside 
            className="flex-1 rounded-3xl overflow-hidden relative shadow-lg bg-black/10 cursor-pointer"
            onClick={handlePanelClick}
          >
            <div key={`media-${activeOption}`} className="absolute inset-0">
              {(() => {
                const active = displayOptions[activeOption];
                if (active?.videoUrl) {
                  return (
                <video
                ...
                <OptimizedImage
                  ...
                  alt={`${active?.main || 'Bannière'} media`}
