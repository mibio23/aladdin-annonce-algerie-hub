...
import { Upload, Plus, Store, AlertCircle, Globe } from 'lucide-react';
...
const CreateShopPage: React.FC = () => {
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      try {
        ...
        logger.error('Erreur compression logo multilingue:', err);
  ...
  const handleProductImagesUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      const remainingSlots = 8 - productFiles.length;
      ...
      const compressedFiles = await Promise.all(
        filesToProcess.map(async (file) => {
          try {
            ...
            logger.error('Erreur compression image produit multilingue:', err);
            return file;
          }
        })
      );
