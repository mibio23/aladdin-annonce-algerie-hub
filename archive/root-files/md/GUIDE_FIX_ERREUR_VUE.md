# Correction de l'Erreur de Dépendance de Vue

L'erreur que vous avez rencontrée (`cannot alter type of a column used by a view...`) est normale : PostgreSQL empêche de modifier une colonne si elle est utilisée par une vue (ici `announcements_safe`).

## Solution

J'ai créé un nouveau script `04-fix-schema-with-views.sql` qui gère ce problème en suivant ces étapes :
1.  **Supprime temporairement** les vues `announcements_safe` et `announcements_public`.
2.  **Modifie la table** `announcements` (ajoute les colonnes et convertit les IDs en TEXT).
3.  **Recrée les vues** proprement.

### Instructions

1.  Ouvrez l'Éditeur SQL de Supabase.
2.  Copiez-collez le contenu du fichier `04-fix-schema-with-views.sql` (ci-dessous).
3.  Exécutez le script.

### Contenu du script (`04-fix-schema-with-views.sql`)
