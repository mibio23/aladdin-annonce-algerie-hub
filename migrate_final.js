// Script de migration final avec IDs numériques
...
/**
 * Convertir un ID texte en numérique unique
 */
function textToNumericId(textId, language) {
    // Créer un hash simple du texte + langue
    let hash = 0;
    ...
    // Prendre les 8 derniers chiffres et s'assurer qu'ils sont positifs
    const numericId = Math.abs(hash % 100000000);
    
    // Éviter les IDs trop petits (commencer à 1000)
    return numericId < 1000 ? numericId + 1000 : numericId;
}
    ...
    try {
        ...
        return matches.map(stmt => {
            ...
            if (valuesMatch) {
                ...
                // Convertir l'ID texte en numérique
                const numericId = textToNumericId(id, language);
                const numericParentId = parentId === 'NULL' ? null : 
                    (parentId.startsWith('\'') ? textToNumericId(parentId.slice(1, -1), language) : parseInt(parentId));
                ...
                return {
                    ...
                    langue: language,  // Utiliser la colonne "langue"
...
async function migrateFile(filePath) {
    ...
    for (let i = 0; i < categories.length; i += batchSize) {
        ...
        try {
            ...
            if (error) {
                ...
                process.stdout.write(`✅ ${successCount}/${categories.length} créées\r`);
            }
        } catch (error) {
    ...
    console.log(`\n✅ ${fileName}: ${successCount} succès, ${errorCount} erreurs`);
    return { successCount, errorCount, total: categories.length };
}
